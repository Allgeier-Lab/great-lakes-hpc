...
Run `file.path(R.home("bin"), "Rscript")` on HPC and use the returning path as `rscript_path` argument. The path should be `"/sw/arcts/centos7/stacks/gcc/8.2.0/R/4.1.0/lib64/R/bin/Rscript"` for all users. You need this in the `slurm_apply`/`slurm_map`
function in your local `R` session later.

That's so far everything we need to setup on the HPC! The next steps are on your local disk again.

## rslurm template (This can be skipped)
...
Prepare your local `R` script (see example below) and run all the code. This will create a `_rslurm_*` folder in your local working directory.

Take this entire folder and use a program such as _FileZilla_ or _CyberDuck_ to copy the it to HPC for example on _FileZilla_, go to `File` > `Site Manager` > and log in to HPC

File server: sftp://greatlakes-xfer.arc-ts.umich.edu

Now, go back tp the _Terminal_ app, login to the HPC (if not logged in already) and navigate to the just copied `_rslurm_*` folder by typen `cd <foldername>`. Once in that folder, to submit a job to the cluster, type `sbatch submit.sh` in the _Terminal_.

Move back to yur home directory by just typing `cd` and check if all jobs are either running, or pending by useing the previously explaine `jobs_*` commands. 

# If jobs fail...which they probably will...

If jobs fail with an error (see `jobs_info`), navigate back to the `_rslurm_*` folder using `cd <foldername>` again and list all files using the `ll` command. There should be either a (or several) `*.out` or `*.log` files (e.g. `slurm_1.out` or `error.log`). Print these files on the console by typing `cat <filename>` (e.g., `cat slurm_1.out`) and check why the job failed. Try to fix the issue.
...
If you didn't write directly into your home directory in your cluster function, copy the entire `_rslurm_*` folder to your local disk by replacing the original folder. Then, you can use `get_slurm_out(sjob, "raw")` to collect the results in the same `R` session.

If you wrote the results directly into your home directory, just collect the results yourself. 

Make sure to delete all `_rslurm_*` folders once you have all results.
